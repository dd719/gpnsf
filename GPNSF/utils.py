...
def to_torch_tensor(x, device, dtype=torch.float32):
    """Convert input to a torch tensor on specified device and dtype."""
...
def add_latent_to_adata(adata, model):
	"""Add normalized latent representation to AnnData object."""

	with torch.no_grad():
		n = model.S.size(0)
		K_model = model.K
		H_mu = torch.empty(n, K_model, device=model.device, dtype=model.dtype)
